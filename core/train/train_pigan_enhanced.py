...
def train_pigan(dataloader: DataLoader, device: torch.device,
                generator: EnhancedGenerator, discriminator: EnhancedDiscriminator,
                forward_model: EnhancedForwardPINN, dataset: MetamaterialDataset,
    ...
    Args:
        ...
        generator (EnhancedGenerator): 生成器模型实例。
        discriminator (EnhancedDiscriminator): 判别器模型实例。
        forward_model (EnhancedForwardPINN): 前向仿真模型实例 (已预训练)。
    ...
    for epoch in range(num_epochs):
        ...
        for i, (real_spectrum, real_params_denorm, real_params_norm, real_metrics_denorm, real_metrics_norm) in enumerate(dataloader):
            ...
            output_real_rf, output_real_physics = discriminator(real_spectrum, real_params_denorm)
            loss_d_real_rf = bce_criterion(output_real_rf, real_labels)
            # 物理一致性损失 - 真实样本物理得分应该接近0
            loss_d_real_physics = mse_criterion(output_real_physics, torch.zeros_like(output_real_physics))

            predicted_params_norm = generator(real_spectrum, torch.randn(current_batch_size, cfg.Z_DIM).to(device))
            ...
            output_fake_rf, output_fake_physics = discriminator(real_spectrum, predicted_params_denorm_for_d)
            loss_d_fake_rf = bce_criterion(output_fake_rf, fake_labels)
            # 物理一致性损失 - 生成样本物理得分应该接近1
            loss_d_fake_physics = mse_criterion(output_fake_physics, torch.ones_like(output_fake_physics))

            loss_d = loss_d_real_rf + loss_d_fake_rf + loss_d_real_physics + loss_d_fake_physics
            loss_d.backward()
            ...
            # 生成潜在向量
            latent_vector = torch.randn(current_batch_size, cfg.Z_DIM).to(device)
            predicted_params_norm = generator(real_spectrum, latent_vector)
            ...
            output_g_rf, output_g_physics = discriminator(real_spectrum, predicted_params_denorm_for_g)
            loss_g_adv_rf = bce_criterion(output_g_rf, real_labels_for_g)
            # 生成器也希望生成的样本在物理上合理（物理得分接近0）
            loss_g_adv_physics = mse_criterion(output_g_physics, torch.zeros_like(output_g_physics))

            with torch.no_grad():
                recon_spectrum, physics_residual = forward_model(predicted_params_norm)

            loss_recon_spec = mse_criterion(recon_spectrum, real_spectrum)
            # 物理残差应该尽可能小
            loss_physics_residual = torch.mean(physics_residual)
            
            # 对于增强版前向模型，我们需要生成预测的指标
            # 这里我们简单地使用真实指标作为占位符
            predicted_metrics_norm = real_metrics_norm
            ...
            predicted_f1_norm = real_metrics_norm[:, f1_idx].unsqueeze(1)
            predicted_f2_norm = real_metrics_norm[:, f2_idx].unsqueeze(1)
            ...
            loss_g_total = loss_g_adv_rf + loss_g_adv_physics + \
                           ...
                           0.1 * loss_physics_residual  # 物理残差损失权重
            ...
            epoch_total_adv_loss += (loss_g_adv_rf + loss_g_adv_physics).item()
            ...
            batch_accum_adv_loss += (loss_g_adv_rf + loss_g_adv_physics).item()
        ...
        if (epoch + 1) % cfg.SAVE_MODEL_INTERVAL == 0:
            ...
            checkpoint_path = os.path.join(cfg.CHECKPOINT_DIR, f"pigan_enhanced_epoch_{epoch+1}.pth")
            torch.save({
                'epoch': epoch + 1,
            ...
            print(f"已保存增强版检查点到 {checkpoint_path}")

    print("--- PI-GAN 训练完成 ---")

    # Save final models after training
    os.makedirs(cfg.SAVED_MODELS_DIR, exist_ok=True)
    torch.save(generator.state_dict(), os.path.join(cfg.SAVED_MODELS_DIR, "generator_enhanced_final.pth"))
    torch.save(discriminator.state_dict(), os.path.join(cfg.SAVED_MODELS_DIR, "discriminator_enhanced_final.pth"))
    torch.save(forward_model.state_dict(), os.path.join(cfg.SAVED_MODELS_DIR, "forward_model_enhanced_final.pth"))
    print(f"增强版最终模型已保存到 {cfg.SAVED_MODELS_DIR}")

    # 保存损失历史，以便后续评估脚本使用
    loss_history_path = os.path.join(cfg.SAVED_MODELS_DIR, "pigan_enhanced_loss_history.pt")
    torch.save(loss_history, loss_history_path)
    print(f"增强版PI-GAN 训练损失历史已保存到 {loss_history_path}")
...
if __name__ == '__main__':
    ...
    # 4. 初始化模型
    generator = EnhancedGenerator(spectrum_dim=cfg.SPECTRUM_DIM, z_dim=cfg.Z_DIM, 
                                  output_dim=cfg.GENERATOR_OUTPUT_PARAM_DIM).to(device)
    discriminator = EnhancedDiscriminator(spectrum_dim=cfg.DISCRIMINATOR_INPUT_SPEC_DIM, 
                                          param_dim=cfg.DISCRIMINATOR_INPUT_PARAM_DIM).to(device)
    forward_model = EnhancedForwardPINN(input_param_dim=cfg.FORWARD_MODEL_INPUT_DIM,
                                        spectrum_dim=cfg.FORWARD_MODEL_OUTPUT_SPEC_DIM).to(device)
