...
def pretrain_forward_model(forward_model: EnhancedForwardPINN, dataloader: DataLoader, device: torch.device,
                           num_epochs: int, lr: float, log_interval: int = 10): # <<<<< 新增参数：log_interval
    """
    预训练前向仿真模型 (EnhancedForwardPINN)。

    Args:
        forward_model (EnhancedForwardPINN): 要预训练的 EnhancedForwardPINN 实例。
    ...
    for epoch in range(num_epochs):
        ...
        for i, (real_spectrum, _, real_params_norm, _, _) in enumerate(dataloader):
            # 将数据移动到指定设备
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="预训练增强版前向模型。")
    ...
    print(f"EnhancedForwardPINN 架构:\n{forward_model}")
